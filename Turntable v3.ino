/*
    Name:       Turntable v3.ino
    Created:	4/16/2020 3:38:16 PM
    Author:     ATTIC-WORKSTATI\Attic
*/
// DCC Stepper Motor Controller ( A4988 ) for Turntable Control
// See: https://www.dccinterface.com/how-to/assemblyguide/
// 
// Author: Alex Shepherd 2017-12-04 for example base, modified by Bob Gamble - 2018-2020
// 
// This program requires two Arduino Libraries:
//
...
#define entryStation1 ((FULL_TURN_STEPS * 2) / 36)
#define entryStation2  ((FULL_TURN_STEPS * 1) / 36)
#define entryStation3 0        // home location
#define houseStation1 ((FULL_TURN_STEPS * -7) / 36) - 1820
#define houseStation2 ((FULL_TURN_STEPS * -8) / 36) - 1820
#define houseStation3 ((FULL_TURN_STEPS * -9) / 36) - 1820
#define houseStation4 ((FULL_TURN_STEPS * -10) / 36) - 1820
#define houseStation5 ((FULL_TURN_STEPS * -11) / 36) - 1820
#define houseStation6 ((FULL_TURN_STEPS * -12) / 36) - 1820
#define houseStation7 ((FULL_TURN_STEPS * -13) / 36) - 1820
#define houseStation8 ((FULL_TURN_STEPS * -14) / 36) - 1820
#define houseStation9 ((FULL_TURN_STEPS * -15) / 36) - 1820
#define houseStation10 ((FULL_TURN_STEPS * -16) / 36) - 3641
#define houseStation11 ((FULL_TURN_STEPS * -18) / 36) - 3641
...
void ConfigureStations()
{
	...
	gAddresses[13].address = 512;
	gAddresses[13].stationFront = houseStation11;
	gAddresses[13].stationBack = houseStation10 + (FULL_TURN_STEPS / 2);
}
...
void setup()
{
	...
	Serial.println(F("Turntable Program - www.dccinterface.com"));
...
void loop()
{
	if (bInterruptDetected)
	{
		...
		Serial.println(entryStation3, DEC);

#ifdef STEPPER_ENABLE_PIN
		stepper1.enableOutputs();
#endif
		stepper1.moveTo(entryStation3);
